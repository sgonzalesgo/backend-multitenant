...
class SubjectTypeRepository
{
    ...
    public function create(array $data): SubjectType
    {
        $tenantId = $this->resolveCurrentTenantId();
    ...
    public function update(SubjectType $subjectType, array $data): SubjectType
    {
        $tenantId = $this->resolveCurrentTenantId();
        ...
        if ((string) $subjectType->tenant_id !== (string) $tenantId) {
            abort(404);
        }

        $subjectType->fill([
            'code' => Arr::get($data, 'code', $subjectType->code),
            'name' => Arr::get($data, 'name', $subjectType->name),
            'description' => Arr::get($data, 'description', $subjectType->description),
            'is_active' => Arr::get($data, 'is_active', $subjectType->is_active),
        ]);

        $subjectType->save();

        return $subjectType->refresh();
    }

    public function delete(SubjectType $subjectType): void
    {
        $tenantId = $this->resolveCurrentTenantId();
