...
class ParallelRepository
{
    ...
    public function create(array $data): Parallel
    {
        $tenantId = $this->resolveCurrentTenantId();
        ...
        return $parallel->refresh();
    }

    public function update(Parallel $parallel, array $data): Parallel
    {
        $tenantId = $this->resolveCurrentTenantId();
        ...
        if ((string) $parallel->tenant_id !== (string) $tenantId) {
            abort(404);
        }

        $parallel->fill([
            'code' => Arr::get($data, 'code', $parallel->code),
            'name' => Arr::get($data, 'name', $parallel->name),
            'description' => Arr::get($data, 'description', $parallel->description),
            'is_active' => Arr::get($data, 'is_active', $parallel->is_active),
        ]);
    ...
    public function delete(Parallel $parallel): void
    {
        $tenantId = $this->resolveCurrentTenantId();
    ...
    public function findOrFail(Parallel $parallel): Parallel
    {
        $tenantId = $this->resolveCurrentTenantId();
